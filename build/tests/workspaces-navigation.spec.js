...
test('User can log in and navigate to Workstations page via Topology menu', async ({ page }) => {
    ...
    // Step 4: Confirm successful login by checking for the dashboard heading
    await expect(page.getByRole('heading', { name: /general activity/i })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /workstations/i })).toBeVisible();
